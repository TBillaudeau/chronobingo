    ...
    <div className="flex flex-col items-center w-full max-w-4xl mx-auto pb-12 animate-pop">
      ...
      <header className="sticky top-4 z-40 w-full px-4 mb-8 transition-all">
        {user ? (
          ...
          <div className="w-full flex justify-end animate-slide-down">
            <button
              onClick={() => { hapticClick(); onRequestLogin({}); }}
              className="px-4 py-2 bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white font-bold rounded-full transition-all elastic-active flex items-center gap-2 backdrop-blur-md border border-white/5 text-xs uppercase tracking-wider"
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
              </svg>
              <span>{t(lang, 'lobby.guestLogin') || "Connexion"}</span>
            </button>
          </div>
        )}
      </header>
