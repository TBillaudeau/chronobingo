...
const GameLobby = ({ user, lang, activeGame, onJoinGame, onNavigateToProfile }) => {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-4xl mx-auto px-4 py-8 animate-pop">
      ...
      {user.isGuest && (
          <div className="w-full glass-liquid bg-yellow-500/10 border-yellow-500/30 p-3 rounded-2xl mb-6 flex items-center justify-between animate-slide-up">
              <div className="flex items-center gap-3">
                  <span className="text-2xl">⚠️</span>
                  <p className="text-xs text-yellow-200 font-bold leading-tight">
                      Mode Invité : Tes stats et favoris<br/>ne seront pas sauvegardés.
                  </p>
              </div>
              <button 
                onClick={handleLogout} 
                className="px-4 py-2 bg-yellow-500 text-black text-xs font-black rounded-xl hover:bg-yellow-400 transition-colors"
              >
                  CONNEXION
              </button>
          </div>
      ...
      <div className="flex items-center gap-4 mb-8 w-full glass-liquid p-4 rounded-3xl cursor-pointer group elastic-active hover:border-fuchsia-500/50 transition-all" onClick={() => { hapticClick(); onNavigateToProfile(); }}>
        <div className="relative">
            ...
            <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-slate-900 ${user.isGuest ? 'bg-yellow-500' : 'bg-green-500'}`}></div>
        </div>
        <div className="flex-1">
          <h2 className="text-2xl font-black text-white tracking-tight">{t(lang, 'lobby.hello')}, {user.name} !</h2>
          <p className="text-fuchsia-300 text-sm font-medium">{user.isGuest ? 'Invité(e)' : 'Membre VIP'}</p>
        </div>
          ...
          <div className="w-full mt-4">
             ...
             <div className="flex w-full shadow-xl rounded-2xl overflow-hidden">
                  <input 
                    type="text" 
                    placeholder="ABCDEF"
                    className="flex-1 bg-black/20 text-white px-4 py-4 focus:outline-none uppercase font-mono tracking-[0.2em] text-center text-lg placeholder-slate-600 font-bold border-y border-l border-white/10 rounded-l-2xl"
                    value={joinId}
                    onChange={(e) => {
                  ...
                  <button 
                    ...
                    className="bg-cyan-500 hover:bg-cyan-400 disabled:opacity-50 text-slate-900 font-bold px-6 transition-colors elastic-active rounded-r-2xl border-y border-r border-cyan-400/50"
                  >
                    {loading ? '...' : 'GO'}
                  </button>
             </div>
