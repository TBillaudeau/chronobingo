...
function SearchDialog({ onSelectSong }) {
    ...
    return (
        <div className="flex items-center justify-center min-h-screen">
            <div className="w-full max-w-sm bg-gray-800 p-8 rounded-lg shadow-2xl">
                <h2 className="text-2xl font-bold text-center text-white mb-6">Rejoindre la partie <span className="text-pink-500">{gameId}</span></h2>
    ...
    <main className="container mx-auto p-2 sm:p-4">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              ...
              <DialogTrigger asChild>
                ...
                >
                  {song ? (
                    <>
                      <div 
                        className="absolute inset-0 bg-cover bg-center rounded-lg opacity-20 group-hover:opacity-30 transition-opacity"
                        style={{ backgroundImage: `url(${song.album.cover_medium})` }}
                      ></div>
                      <div onClick={(e) => { e.stopPropagation(); toggleSongChecked(index); }} className="relative z-10 w-full h-full flex flex-col items-center justify-center p-1 bg-black/20 rounded-lg">
                        <p className={`font-bold text-[10px] sm:text-sm leading-tight text-shadow-lg ${song.checked ? 'text-white' : 'text-white'}`}>{song.title}</p>
                        <p className={`text-[9px] sm:text-xs text-gray-300 text-shadow-md ${song.checked ? 'text-gray-100' : ''}`}>{song.artist.name}</p>
                      </div>
        ...
        <DialogContent className="bg-gray-900 border-gray-700 text-white">
          <SearchDialog onSelectSong={handleSelectSong} />
        </DialogContent>
      </Dialog>

      <div className="mt-12 sm:mt-16">
        <h2 className="text-xl sm:text-2xl font-semibold text-center mb-4">Grilles des autres joueurs</h2>
        {otherPlayers.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8">
                {otherPlayers.map(p => (
                    <div key={p.id}>
                        <h3 className="text-lg sm:text-xl font-bold text-center mb-2">{p.username} - <span className="text-pink-500">{p.score} pts</span></h3>
            ...
            <p className="text-center text-gray-500">Vous êtes seul dans cette partie. Partagez le lien pour inviter des amis !</p>
        )}
      </div>
    </main>
  );
